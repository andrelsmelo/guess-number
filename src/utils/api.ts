...
export async function getRanking() {
  try {
    const result = await sql`SELECT username, SUM(score) as qtd FROM ranking GROUP BY username ORDER BY qtd DESC LIMIT 10`;
    return { status: 200, data: result.rows };
  } catch (error) {
    return { status: 500, error: 'An error occurred while fetching the ranking.' };
  }
}
